...
class KeyboardEndEffectorTeleop(KeyboardTeleop):
    ...
    def _on_press(self, key):
        ...
        if hasattr(key, "char") and key.char is not None:
            self.event_queue.put((key.char, True))
            # Immediately capture character key presses for event detection.
            # We cannot rely on the polled state in get_action() because a quick
            # press-and-release may already be fully processed (final state=False)
            # by the time the loop calls _drain_pressed_keys().
